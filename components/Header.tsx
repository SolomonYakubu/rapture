...
import corner from "../public/images/logo.webp";
  ...
  return (
    <div className="fixed top-0 z-40">
      ...
      >
        ...
        <ul className="  md:justify-between  md:flex hidden">
          {home &&
            list.map(([name, link], index) => (
              <Link
                className="m-2 text-white font-bold  cursor-pointer hover:text-blue-400 font-[Montserrat]"
                key={index}
                activeClass="none"
                to={`${name}`}
                spy={true}
        ...
        >
          {(isNavOpen && <MdCloseFullscreen size={40} />) || (
      ...
      <div
        className={`md:hidden absolute top-0 z-40 overflow-hidden min-h-screen flex  bg-bg2 shadow-lg w-full items-center justify-center transition-all ease-out duration-300 ${
          (isNavOpen && " scale-100") || "scale-0 py-0"
        }`}
      >
          ...
          {home &&
            list.map(
              ([name, link], index) =>
                (home && (
                  <Link
                    className={`mt-4  text-gray-50  text-center   font-bold  text-2xl font-[Montserrat]`}
                    key={index}
